...
import { Headphones, Phone, Lock, Loader2, ArrowLeft, AlertCircle, Eye, EyeOff } from 'lucide-react'
...
export default function SupportLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch (err: any) { setError(err.message || 'فشل الدخول') }
  }
  ...
  return (
    <div className="min-h-dvh bg-white flex flex-col" dir="rtl">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 -mt-10">
        ...
        <motion.h1 initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="text-2xl font-bold mb-1">AVEX Support</motion.h1>
        <motion.p initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-sm text-gray-500 mb-8 text-center">لوحة موظف الدعم</motion.p>
        ...
        <div className="mt-8 text-center text-xs text-gray-400">
          <p>حسابات تجريبية:</p>
          <p dir="ltr" className="mt-1 font-mono">01500000001 / 01500000002 — 123456</p>
        </div>
      </div>
    </div>
  )
