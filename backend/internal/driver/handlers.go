...
func HandleDriverLogin(w http.ResponseWriter, r *http.Request) {
        ...
        err := shared.DB.QueryRow("SELECT id, name, phone, password_hash, is_active, is_verified, must_change_password FROM drivers WHERE phone = ?", p).Scan(&id, &name, &ph, &hash, &active, &verified, &mustChange)
        ...
        // Mark last_seen
        shared.DB.Exec("UPDATE drivers SET last_seen_at = CURRENT_TIMESTAMP WHERE id = ?", id.String)
...
func HandleDriverChangePassword(w http.ResponseWriter, r *http.Request) {
        ...
        var hash string
        shared.DB.QueryRow("SELECT password_hash FROM drivers WHERE id = ?", c.DriverID).Scan(&hash)
        ...
        shared.DB.Exec("UPDATE drivers SET password_hash = ?, must_change_password = 0 WHERE id = ?", newHash, c.DriverID)
...
func HandleDriverToggleOnline(w http.ResponseWriter, r *http.Request) {
        ...
        json.NewDecoder(r.Body).Decode(&b)
        shared.DB.Exec("UPDATE drivers SET is_online = ?, last_seen_at = CURRENT_TIMESTAMP WHERE id = ?", b.Online, c.DriverID)
...
func HandleDriverUpdateLocation(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.Exec("UPDATE drivers SET lat = ?, lng = ?, location_updated_at = CURRENT_TIMESTAMP, last_seen_at = CURRENT_TIMESTAMP WHERE id = ?", b.Lat, b.Lng, c.DriverID)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}
...
func HandleDriverToggleAutoAccept(w http.ResponseWriter, r *http.Request) {
        ...
        json.NewDecoder(r.Body).Decode(&b)
        shared.DB.Exec("UPDATE drivers SET auto_accept = ? WHERE id = ?", b.AutoAccept, c.DriverID)
...
func HandleDriverGetOffers(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.Exec("UPDATE dispatch_offers SET status = 'expired' WHERE driver_id = ? AND status = 'pending' AND expires_at < CURRENT_TIMESTAMP", c.DriverID)
        ...
        for rows.Next() {
                ...
                shared.DB.QueryRow("SELECT tier_id FROM drivers WHERE id = ?", c.DriverID).Scan(&tierID)
                deliveryDist := 0.0
...
func HandleDriverAcceptOffer(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT order_id, status, expires_at FROM dispatch_offers WHERE id = ? AND driver_id = ?", offerID, c.DriverID).Scan(&orderID, &status, &expiresAt)
        ...
        if time.Now().After(expTime) {
                shared.DB.Exec("UPDATE dispatch_offers SET status = 'expired', responded_at = CURRENT_TIMESTAMP WHERE id = ?", offerID)
                shared.WriteErr(w, 400, "انتهت صلاحية العرض"); return
        }
        // Check if another driver already accepted
        var exists string
        if shared.DB.QueryRow("SELECT driver_id FROM dispatch_offers WHERE order_id = ? AND status = 'accepted'", orderID.String).Scan(&exists) == nil {
                shared.DB.Exec("UPDATE dispatch_offers SET status = 'expired', responded_at = CURRENT_TIMESTAMP WHERE id = ?", offerID)
...
func HandleDriverRejectOffer(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT order_id FROM dispatch_offers WHERE id = ? AND driver_id = ? AND status = 'pending'", offerID, c.DriverID).Scan(&orderID)
        if !orderID.Valid { shared.WriteErr(w, 404, "العرض غير موجود"); return }
        shared.DB.Exec("UPDATE dispatch_offers SET status = 'rejected', responded_at = CURRENT_TIMESTAMP WHERE id = ?", offerID)
        // Increment rejected count
        shared.DB.Exec("UPDATE driver_stats SET rejected_orders = rejected_orders + 1, total_orders = total_orders + 1, updated_at = CURRENT_TIMESTAMP WHERE driver_id = ?", c.DriverID)
...
func HandleDriverGetActiveOrder(w http.ResponseWriter, r *http.Request) {
        ...
        itemRows, _ := shared.DB.Query("SELECT name, price, quantity FROM order_items WHERE order_id = ?", id.String)
        for itemRows.Next() {
...
func HandleDriverPickedUp(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT o.status FROM orders o WHERE o.id = ? AND o.driver_id = ?", orderID, c.DriverID).Scan(&status)
        ...
        shared.DB.QueryRow("SELECT lat, lng FROM drivers WHERE id = ?", c.DriverID).Scan(&dLat, &dLng)
        ...
        shared.DB.QueryRow("SELECT lat, lng FROM restaurants r JOIN orders o ON o.restaurant_id = r.id WHERE o.id = ?", orderID).Scan(&restLat, &restLng)
        ...
        shared.DB.Exec("UPDATE orders SET status = 'picked_up', updated_at = CURRENT_TIMESTAMP WHERE id = ?", orderID)
...
func HandleDriverArrived(w http.ResponseWriter, r *http.Request) {
        ...
        orderID := r.PathValue("id")
        var status sql.NullString
        shared.DB.QueryRow("SELECT status FROM orders WHERE id = ? AND driver_id = ?", orderID, c.DriverID).Scan(&status)
        if !status.Valid { shared.WriteErr(w, 404, "الطلب غير موجود"); return }
        ...
        if status.String == "picked_up" {
                shared.DB.Exec("UPDATE orders SET status = 'on_the_way', updated_at = CURRENT_TIMESTAMP WHERE id = ?", orderID)
        }
...
func HandleDriverDelivered(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT status, location_lat, location_lng, driver_fee FROM orders WHERE id = ? AND driver_id = ?", orderID, c.DriverID).Scan(&status, &cLat, &cLng, &driverFee)
        if !status.Valid { shared.WriteErr(w, 404, "الطلب غير موجود"); return }
        ...
        shared.DB.Exec("UPDATE orders SET status = 'delivered', updated_at = CURRENT_TIMESTAMP WHERE id = ?", orderID)
        // Update driver stats: completed+1, on_time+1 (assume on-time for now), earnings += driver_fee
        shared.DB.Exec(`UPDATE driver_stats SET completed_orders = completed_orders + 1, on_time_count = on_time_count + 1, total_earnings = total_earnings + ?, updated_at = CURRENT_TIMESTAMP WHERE driver_id = ?`,
...
func HandleDriverEarnings(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT COALESCE(SUM(driver_fee), 0), COUNT(*) FROM orders WHERE driver_id = ? AND status = 'delivered' "+periodClause, c.DriverID).Scan(&total, &count)
        shared.WriteJSON(w, 200, map[string]interface{}{
                "period": period,
...
func HandleDriverCreateTicket(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.Exec("INSERT INTO support_tickets (id, driver_id, order_id, type, reason, status) VALUES (?, ?, ?, ?, ?, 'open')", id, c.DriverID, orderID, b.Type, b.Reason)
...
func HandleDriverGetTickets(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil { shared.WriteErr(w, 401, "غير مصرح"); return }
        rows, _ := shared.DB.Query("SELECT id, order_id, type, reason, status, created_at, resolved_at FROM support_tickets WHERE driver_id = ? ORDER BY created_at DESC", c.DriverID)
...
func HandleDriverGetTicket(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT type, reason, status, order_id, created_at, resolved_at FROM support_tickets WHERE id = ? AND driver_id = ?", ticketID, c.DriverID).Scan(&typ, &reason, &status, &oid, &ct, &rt)
        if !typ.Valid { shared.WriteErr(w, 404, "التذكرة غير موجودة"); return }
        rows, _ := shared.DB.Query("SELECT id, sender, body, created_at FROM support_messages WHERE ticket_id = ? ORDER BY created_at ASC", ticketID)
...
func HandleDriverSendMessage(w http.ResponseWriter, r *http.Request) {
        ...
        var status sql.NullString
        shared.DB.QueryRow("SELECT status FROM support_tickets WHERE id = ? AND driver_id = ?", ticketID, c.DriverID).Scan(&status)
        ...
        mid := uuid.New().String()
        shared.DB.Exec("INSERT INTO support_messages (id, ticket_id, sender, body) VALUES (?, ?, 'driver', ?)", mid, ticketID, b.Body)
