...
func HandleMerchantLogin(w http.ResponseWriter, r *http.Request) {
        ...
        err := shared.DB.QueryRow("SELECT m.id, m.name, m.phone, m.password_hash, m.restaurant_id, m.is_active, m.must_change_password FROM merchants m WHERE m.phone = ?", p).Scan(&id, &name, &ph, &hash, &restID, &active, &mustChange)
        ...
        shared.DB.Exec("UPDATE merchants SET last_login = CURRENT_TIMESTAMP WHERE id = ?", id.String)
        token, _ := shared.GenerateMerchantJWT(id.String, restID.String, ph.String, name.String)
        // Get restaurant info
        var rName, rNameAr sql.NullString
        shared.DB.QueryRow("SELECT name, name_ar FROM restaurants WHERE id = ?", restID.String).Scan(&rName, &rNameAr)
...
func HandleMerchantMe(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT m.id, m.name, m.phone, m.restaurant_id, m.is_active, m.must_change_password, 0 FROM merchants m WHERE m.id = ?", c.MerchantID).Scan(&id, &name, &ph, &restID, &active, &mustChange, &autoAccept)
        ...
        shared.DB.QueryRow("SELECT name, name_ar, description_ar, lat, lng, rating, rating_count, is_active, is_pro, delivery_time_min, delivery_time_max, delivery_fee, min_order FROM restaurants WHERE id = ?", restID.String).Scan(&rName, &rNameAr, &rDesc, &rLat, &rLng, &rRating, &rRC, &rActive, &rPro, &rDtMin, &rDtMax, &rDFee, &rMinOrd)
        shared.WriteJSON(w, 200, map[string]interface{}{
                "id": id.String, "name": name.String, "phone": ph.String,
...
func HandleMerchantChangePassword(w http.ResponseWriter, r *http.Request) {
        ...
        var hash string
        shared.DB.QueryRow("SELECT password_hash FROM merchants WHERE id = ?", c.MerchantID).Scan(&hash)
        ...
        shared.DB.Exec("UPDATE merchants SET password_hash = ?, must_change_password = 0 WHERE id = ?", newHash, c.MerchantID)
...
func HandleMerchantGetOrderItems(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT restaurant_id FROM orders WHERE id = ?", orderID).Scan(&restID)
        if !restID.Valid || restID.String != c.RestaurantID { shared.WriteErr(w, 403, "غير مصرح"); return }
        rows, _ := shared.DB.Query("SELECT id, menu_item_id, name, price, quantity FROM order_items WHERE order_id = ?", orderID)
...
func HandleMerchantUpdateOrderStatus(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT restaurant_id, status FROM orders WHERE id = ?", orderID).Scan(&restID, &currStatus)
        if !restID.Valid || restID.String != c.RestaurantID { shared.WriteErr(w, 403, "غير مصرح"); return }
        // If merchant rejects (special case): create support ticket
        if b.Status == "rejected" {
                shared.DB.Exec("UPDATE orders SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = ?", orderID)
                shared.WriteJSON(w, 200, map[string]interface{}{"success": true, "status": "cancelled"})
                return
        }
        shared.DB.Exec("UPDATE orders SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", b.Status, orderID)
...
func HandleMerchantUpdateMenuItem(w http.ResponseWriter, r *http.Request) {
        ...
        // verify ownership
        var restID sql.NullString
        shared.DB.QueryRow("SELECT restaurant_id FROM menu_items WHERE id = ?", id).Scan(&restID)
        if !restID.Valid || restID.String != c.RestaurantID { shared.WriteErr(w, 403, "غير مصرح"); return }
        ...
        if b.IsPopular != nil { shared.DB.Exec("UPDATE menu_items SET is_popular = ? WHERE id = ?", *b.IsPopular, id) }
        if b.IsAvailable != nil { shared.DB.Exec("UPDATE menu_items SET is_available = ? WHERE id = ?", *b.IsAvailable, id) }
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}
func HandleMerchantDeleteMenuItem(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.QueryRow("SELECT restaurant_id FROM menu_items WHERE id = ?", id).Scan(&restID)
        if !restID.Valid || restID.String != c.RestaurantID { shared.WriteErr(w, 403, "غير مصرح"); return }
        shared.DB.Exec("DELETE FROM menu_items WHERE id = ?", id)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}
...
func HandleMerchantGetHours(w http.ResponseWriter, r *http.Request) {
        c := shared.GetUser(r)
        if c == nil || !c.IsMerchant { shared.WriteErr(w, 401, "غير مصرح"); return }
        rows, _ := shared.DB.Query("SELECT id, day_of_week, open_time, close_time, is_open FROM store_hours WHERE restaurant_id = ? ORDER BY day_of_week ASC", c.RestaurantID)
...
func HandleMerchantTogglePause(w http.ResponseWriter, r *http.Request) {
        ...
        json.NewDecoder(r.Body).Decode(&b)
        shared.DB.Exec("UPDATE restaurants SET is_active = ? WHERE id = ?", b.IsActive, c.RestaurantID)
