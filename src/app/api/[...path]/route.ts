...
// Fallback data for when Go backend is not running
function getFallbackData(path: string): unknown | null {
  if (path === 'health') {
    return { service: 'avex-api', status: 'degraded' }
  }
  if (path === 'menu') {
    return { categories: [] }
  }
...
async function proxyRequest(
  ...
  try {
    ...
    // Go backend is down - return fallback data for GET requests
    if (method === 'GET') {
      const fallback = getFallbackData(pathStr)
      if (fallback) {
        return NextResponse.json(fallback)
      }
    }
    return NextResponse.json(
      { error: 'خدمة الـ API غير متاحة حالياً. يرجى المحاولة مرة أخرى.' },
      { status: 503 }
    )
  }
}
