        ...
        {!order ? (
          <div className="bg-white rounded-2xl border p-6 mb-6">
            <div className="text-center mb-6">
              <div className="w-16 h-16 mx-auto rounded-2xl bg-violet-50 flex items-center justify-center mb-3"><Package className="w-8 h-8 text-violet-600" /></div>
            ...
            <div className="space-y-3">
              ...
              <Button onClick={() => fetchOrder(searchOrderNumber)} disabled={loading} className="w-full h-12 rounded-xl font-bold bg-violet-600 hover:bg-violet-700">{loading ? 'جاري البحث...' : 'تتبع الطلب'}{!loading && <Search className="w-4 h-4 mr-1" />}</Button>
            </div>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="bg-white rounded-2xl border p-5">
              ...
              <div className="flex items-center justify-between pt-3 border-t">
                ...
                <span className={`text-sm font-bold px-3 py-1 rounded-full ${isCancelled ? 'bg-red-100 text-red-700' : 'bg-violet-100 text-violet-700'}`}>{order.status === 'cancelled' ? 'ملغى' : order.status === 'rejected' ? 'مرفوض' : STEPS[currentStep]?.label || order.status}</span>
              </div>
            </div>
            {!isCancelled ? (
              <div className="bg-white rounded-2xl border p-5">
                  ...
                  {STEPS.map((step, idx) => {
                    ...
                    return (
                      <div key={step.id} className="flex gap-3 pb-6 last:pb-0 relative">
                        {idx < STEPS.length - 1 && <div className={`absolute right-5 top-10 w-0.5 h-[calc(100%-2rem)] ${idx < currentStep ? 'bg-violet-600' : 'bg-gray-200'}`} />}
            ...
            <div className="bg-white rounded-2xl border p-5">
              ...
              <div className="border-t mt-4 pt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between font-bold text-base pt-1"><span>الإجمالي</span><span className="text-violet-600">{order.total?.toFixed(2)} ج.م</span></div>
              </div>
            </div>
