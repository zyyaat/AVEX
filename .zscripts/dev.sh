...
# Start Go API backend (AVEX) - MUST use setsid+nohup for persistence
log_step_start "Starting AVEX Go API"
echo "[API] Starting Go backend on port 8080..."
...
if [ -f "./avex-api" ]; then
    ...
    if curl -s http://localhost:8080/api/health > /dev/null 2>&1; then
        echo "[API] ✅ Go API healthy"
    else
        echo "[API] ⚠️ Go API not responding yet"
    fi
else
    echo "[API] ⚠️ avex-api binary not found, skipping"
fi
cd "$PROJECT_DIR"
log_step_end "Starting AVEX Go API"
